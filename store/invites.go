...
type InviteCode struct {
	ID          uuid.UUID
	InviterID   uuid.UUID
	Code        string // Cryptographic token (not stored in DB, generated on-the-fly)
...
// The cryptographic token is generated by the handler using the InviteTokenGenerator.
// This method stores the invite in the database for tracking.
func (db *DB) CreateInviteCode(ctx context.Context, inviterID uuid.UUID, email string, inviteeName *string) (*InviteCode, error) {
	invite := &InviteCode{}
	err := db.pool.QueryRow(ctx, `
		INSERT INTO invite_codes (inviter_id, email, invitee_name)
		VALUES ($1, $2, $3)
		RETURNING id, inviter_id, email, invitee_name, status, created_at, expires_at
	`, inviterID, email, inviteeName).Scan(
		&invite.ID, &invite.InviterID, &invite.Email,
...
func (db *DB) GetPendingInviteByUsernames(ctx context.Context, inviterUsername, inviteeEmail string) (*InviteCode, error) {
	...
	err := db.pool.QueryRow(ctx, `
		SELECT ic.id, ic.inviter_id, ic.email, ic.invitee_name, ic.status, ic.created_at, ic.expires_at
...
func (db *DB) UseInvite(ctx context.Context, inviteID, usedByID uuid.UUID) (*InviteCode, error) {
	...
	err := db.pool.QueryRow(ctx, `
		...
		RETURNING id, inviter_id, email, invitee_name, status, used_at, used_by, created_at, expires_at
...
// Note: The Code field is not returned from DB; cryptographic tokens are generated on-demand.
